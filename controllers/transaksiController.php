    ...
    if (isset($_POST["type"])) {
        if ($_POST["type"] == "insert") {
            ...
            for ($i = 0; $i < count($data); $i++) {
                ...
                // if ($i == count($data) - 1) {
                //     $param = $param . "'" . $data[$i]["kode"] . "'";
                // } else {
                //     $param = $param . "'" . $data[$i]["kode"] . "'" . ', ';
                // }

                // move to frame transaksi
                $crud->execute("INSERT INTO frame_transaksi (kode_detail_pesanan, harga) SELECT kode_pesanan, harga FROM keranjang_frame WHERE kode_pesanan = '" . $data[$i]["kode"] . "'");
