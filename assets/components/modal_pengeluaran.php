  ...
  <div id="modalkonten" class="flex flex-col fixed z-[51] left-[50%] top-[50%] -translate-y-[50%] -translate-x-[50%] justify-center size-modal-add items-center scale-0 transition ease-in-out w-[380px] shadow-xl bg-white rounded-xl overflow-hidden">
    ...
    <div id="formRestock" class="flex flex-col overflow-y-scroll scrollbar-hide w-full px-3 h-[60vh]">
      <div class="h-12 flex mt-8 items-center  px-5">
        <h1 class="w-1/2 font-semibold text-sm">Tanggal</h1>
        <div class=" h-[50px] w-full border border-[#C9C9C9] rounded-lg ml-4 overflow-hidden">
          <input type="date" name="" class="h-full w-full" id="daterestok">
        </div>
      </div>
      ...
      <div class=" flex w-full items-center  px-5 mt-7">
        ...
        <select name="supplier" id="supplier" class="h-full w-full outline-0 border-0 px-4 bg-white">
              ...
              ?>            
              <option value="<?php echo $data['Id_Supplier']?>"><?php echo $data['Nama_Supplier'] . " ( ".$data['Keterangan']." )" ?></option>
              <?php } ?>
              </select>
        </div>
      </div>

      <div class=" flex w-full items-center  px-5 mt-7 mb-10">
    ...
    <div class="flex flex-row justify-center w-full px-[30px] py-[20px] font-ex-semibold text-white border-t-2">
      <div class="cursor-pointer flex hover:bg-[#E7261F] hover:transition hover:duration-[0.3s] justify-center items-center h-[40px] w-[103px] bg-[#F35E58] rounded-md" id="btn_batal">
        Batal
      </div>
      <div class="cursor-pointer flex hover:bg-[#06A981] hover:transition hover:duration-[0.3s] justify-center items-center ml-4 h-[40 px] w-[103px] bg-[#3DBD9E] rounded-md" id="btn_tambah">
        Tambah
      </div>
    </div>
    <!-- end button -->
      ...
      if (jenis == "produk") {
        ...
        $.ajax({
          ...
          success: function(res) {
            ...
            for (let index = 0; index < data.length; index++) {
              //console.log(data);
              isi += '<option value=' + data[index].kode_frame + '>'  + data[index].merk + ' ( ' + data[index].kode_frame + ' )</option>';
            }
        ...
        $.ajax({
          ...
          success: function(res) {
            ...
            for (let index = 0; index < data.length; index++) {
              //console.log(data);
              isi += '<option value=' + data[index].kode_barang + '>' + data[index].nama_barang + '</option>';
            }
        ...
        $.ajax({
          ...
          success: function(res) {
            ...
            for (let index = 0; index < data.length; index++) {
              //console.log(data);
              isi += '<option value=' + data[index].kode_perlengkapan + '>' + data[index].nama_perlengkapan + '</option>';
            }
