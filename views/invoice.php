...
<body class="bg-[#F0F0F0] font-ex-color box-border text-[#343948]">
              ...
              <?php foreach ($dataProses as $index) : ?>
                <tr>
                  ...
                  <td class="p-3 text-sm tracking-wide text-center">
                    ...
                    <button <?php if ($index["status_pengiriman"] === "terima") : ?> disabled <?php endif ?> onclick="show_tracking('<?= $index['kode_pesanan'] ?>', '<?= $index['status_pengiriman'] ?>')">
                      <svg width="35" height="35" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <script>
    ...
    function show_tracking(id, statusDB) {
      ...
      let dataStatus = ["produksi", "kemas", "kirim", "terima"];
      let kontenHtml = ""
    ...
    function show_detail(id) {
      ...
      $.ajax({
        ...
        success: function(res) {
          ...
          kontenHtml += '<div class="w-[10%] py-1 flex justify-center">:</div>'
          kontenHtml += '<div class="w-[50%] py-1">' + formatRupiah(final_data_pembayaran.total_bayar, "Rp. ") + '</div>'
          kontenHtml += '</div>'
          kontenHtml += '<div class="flex flex-row w-full">'
          kontenHtml += '<div class="w-[40%] py-1">Pembayaran 1</div>'
          kontenHtml += '<div class="w-[10%] py-1 flex justify-center">:</div>'
          kontenHtml += '<div class="w-[50%] py-1">Blm Dibuat :)</div>'
          kontenHtml += '</div>'
          kontenHtml += '<div class="flex flex-row w-full">'
